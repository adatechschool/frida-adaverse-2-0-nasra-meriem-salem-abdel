...
export default async function ProductsPage({ params }: ProductsPageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      {/* ❤️ FAVORI */}
{connectedUserId ? (
  <Favclient productId={product.id} userId={connectedUserId} />
) : (
    ...
    Connectez-vous pour ajouter ce produit à vos favoris
      ...
      <section className="mt-8">
        ...
        {comments.map((comment) => (
          <div key={comment.id} className="mb-4 rounded border p-3">
            <small className="mb-1 font-medium">{comment.authorName}</small>
