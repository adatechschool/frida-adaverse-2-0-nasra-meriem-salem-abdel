...
const Fav: React.FC<FavProps> = ({ id, isLiked, onToggleLike }) => {
  ...
  return (
    <div
      ...
      className="flex items-center justify-center w-8 h-8 bg-gray-900 rounded-full p-1 cursor-pointer"
      onClick={() => {
        try { console.debug('[Fav] click', { id, isLiked }); } catch {}
        onToggleLike();
      }}
      role="button"
    ...
    >
      ...
      <label htmlFor={uniqueId} className="flex items-center justify-center">
        <svg
          viewBox="0 0 512 512"
         
          className={`w-5 h-5 fill-white transition-opacity duration-300 ${isLiked ? "hidden" : "block"}`}
        >
        ...
        <svg
          ...
          className={`w-5 h-5 fill-red-600 transition-transform duration-300 ${isLiked ? "block animate-wiggle" : "hidden"}`}
        >
